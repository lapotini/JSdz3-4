...
console.log(test.questions[i].title);
  var question = document.createElement('h2');

  question.innerHTML = test.questions[i].title;

  form.appendChild(question);


  for(var y = 0; y < test.questions[i].answer.length; y++){
  console.log(test.questions[i].answer[y].text);
    var label = document.createElement('label');
    var input = document.createElement('input');
    input.type = 'radio';
    input.name = 'vopros' + i;
    var answer = document.createElement('span');
    answer.innerHTML = test.questions[i].answer[y].text;
    answer.color = 'red';
